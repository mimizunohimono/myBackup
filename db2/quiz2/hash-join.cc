...
main(void)
{
  ...
  while (true) {
    ...
    while (true) {
      ...
      ns = read(sfd, bufS, sizeof(TUPLE)); // read 1 tuple of S
      ...
      int hash = bufS[0].val % NB_BUCKET; // search the correct bucket in index
      for (HASHOBJ *o = bucket[hash].head.nxt; o; o = o->nxt) { // search correct val in 1 bucket in index
	if (o->tuple.val == bufS[0].val) { // if values are same, join R and S
	  result.rkey = o->tuple.key;
	  result.rval = o->tuple.val;
	  result.skey = bufS[0].key;
	  result.sval = bufS[0].val;
	  countVal++;
	}
      }
